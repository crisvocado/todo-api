...
def test_emit_log_signature_is_the_contract():
    # Pinned on purpose: the demo scenario and anything else that reports to
    # logcore call emit_log this way. If an onboarding regenerates this module
    # with another shape, it fails here instead of somewhere far away.
    assert list(inspect.signature(emit_log).parameters) == [
        "severity",
        "message",
        "error",
        "trace_id",
...
def test_emit_log_works_without_an_exception(capsys, monkeypatch):
    # A bug that computes the wrong value never raises, so `error` is optional.
    monkeypatch.setenv("LOGCORE_SERVICE_ID", "service-under-test")

    emit_log("ERROR", "count mismatch", context={"reported": 2, "actual": 0})

    entry = json.loads(capsys.readouterr().out.strip())
    assert entry["message"] == "count mismatch"
    assert entry["context"] == {"reported": 2, "actual": 0}
